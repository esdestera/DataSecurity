...
void decryptText(char key[], char cypheredText[]) {
	...
	for (i = 0; i < strlen(cypheredText); i++) {
		if (i >= keySize) {
			key[i] = text[i - keySize];
		}

		int line = (int)key[i] - 65;    //(int)key[i % strlen(key)] - aici iau numarul ascii pentru litera, stiind ca matricea contine pe linie albabetul trebuie sa merg pe linia corespunzatoare literei
		char element = cypheredText[i];
		for (j = 0; j < MATRIXDIMENSION; j++) {
			if (matrix[line][j] == element) {
				text[i] = (char)j + 65;		
				break;//am gasit coloana, luam litera care se afla la j + 65
			}
		}		
	}	
}
...
void createKey(char key[], char plainText[]) {
	int k = strlen(key);
	int i;
	...
	while (k < strlen(plainText)) {
		pass[k] = plainText[k - strlen(key)];
		k++;		
	}
}
...
int main()
{
	...
	while (fgets(plainText, 1000, g) != NULL && fgets(key, 1000, f) != NULL && fgets(cypheredText, 1000, h)) {
		...
		createKey(key, plainText);
		encryptText(pass, plainText);		
